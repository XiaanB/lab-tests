...
Note: Students should use these examples to verify their
implementation
...
expected_score = 190
print_game_results("Example Game", rolls, expected_score,
actual_score)
return actual_score
def perfect_game():
...
actual_score = game.score()
expected_score = 300
print_game_results("Perfect Game", rolls, expected_score,
actual_score)
return actual_score
def all_spares():
...
actual_score = game.score()
expected_score = 150
print_game_results("All Spares Game", rolls, expected_score,
actual_score)
return actual_score
def gutter_game():
...
actual_score = game.score()
expected_score = 0
print_game_results("Gutter Game", rolls, expected_score,
actual_score)
return actual_score
def regular_game():
...
"""
game = BowlingGame()
rolls = [3, 4, 2, 5, 1, 6, 4, 2, 8, 1, 7, 1, 5, 3, 2, 3, 4, 3,
2, 6]
for pins in rolls:
game.roll(pins)
# Calculate the final score
actual_score = game.score()
expected_score = 72
print_game_results("Regular Game", rolls, expected_score,
actual_score)
return actual_score
def main():
...
print("=====================")
print("These examples demonstrate how bowling scoring should
work.")
print("The 'Correct implementation' indicator shows if our
current")
print("implementation calculates the score correctly.")
print("\nStudents should ensure their fixed implementation
correctly")
...
main()
        Returns:
        Bonus score for the strike
        """
        if frame_index + 1 < len(self.rolls):
            bonus = self.rolls[frame_index + 1]
            if bonus == 10 and frame_index + 2 < len(self.rolls):
                bonus += self.rolls[frame_index + 2]
            elif frame_index + 2 < len(self.rolls):
                bonus += self.rolls[frame_index + 2]
            return bonus
        return 0
    def _spare_bonus(self, frame_index):
        ...
        Args:
        frame_index: Index of the first roll in the spare frame
        Returns:
